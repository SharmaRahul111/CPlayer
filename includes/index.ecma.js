...
function scansong(url9){var xhr = new XMLHttpRequest();xhr.open("GET","scanner.php?songs="+url9,true);xhr.onload = function(){if(id("song").innerHTML != this.responseText){id("song").innerHTML = this.responseText;let moreBtnSong = qryAll(".more-btn");
moreBtnSong.forEach((moreBtn)=>{
	moreBtn.onclick =()=>{
...
function hide(elm,index){elm.style.transition = ".4s";elm.style.height = 0;if(index == playlist.length -1){playlist.pop();}else{playlist.splice(index,1);}if(currIndex >= index && currIndex != 0){currIndex -=1;}setTimeout(()=>{elm.style.display = "none";updatePlaylist();},450);}
/*slider functions*/
...
function slideUI(){
	...
	playlistElm.forEach((elm,index)=>{
		elm.ontouchstart=(e)=>{touchStartPlDiv = e.touches[0].clientX;touchEndPlDiv = e.touches[0].clientX;};
		elm.ontouchmove=(e)=>{elm.style.left = Number(touchStartPlDiv - e.touches[0].clientX).toFixed(2)*(-1)+"px";touchEndPlDiv = e.touches[0].clientX;};
		elm.ontouchend=(e)=>{var width = getComputedStyle(elm).width.substr(0,getComputedStyle(elm).width.length - 2);var x = touchEndPlDiv;if(touchStartPlDiv - 50 >= x){elm.style.left -= width;hide(elm,index);}else{elm.style.left = 0+"px";}};
...
id("closeSong").addEventListener("click",()=>{id("songlist").style.left = 100+"vw";id("song").innerHTML = null;});
id("optContainer").addEventListener("click",()=>{setTimeout(()=>{id("optContainer").style.display = "none";},120)},true);
qry("#optList span:nth-child(1)").addEventListener("click",()=>{let urlw = qry("#optList span:nth-child(1)").getAttribute("data-url");playlist.splice(currIndex+1,0,urlw);updatePlaylist();});
qry("#optList span:nth-child(2)").addEventListener("click",()=>{let nurl = qry("#optList span:nth-child(2)").getAttribute("data-url");if(nurl){playlist.push(nurl);updatePlaylist();}});
launcher.addEventListener("click",()=>{var player = id("player");var i = launcher.querySelector("i");var num = -16;if(player.style.top == num+"px"){player.style.top = innerHeight-65+"px";i.className = "fas fa-angle-up";if(location.hash == "#player"){/*location.hash = '';*/}}else{player.style.top = num+"px";i.className = "fas fa-angle-down";}});
audio.addEventListener("timeupdate",()=>{timestamp[1].innerHTML = inMin(audio.duration,1);timestamp[0].innerHTML = inMin(audio.currentTime,0);slider.value = Math.round((audio.currentTime/audio.duration)*500);setBar();});
playpause.addEventListener("click",()=>{if(audio.paused){if(audio.src){playpause.querySelector("i").className = "fas fa-pause";audio.play();}else if(playlist.length !== 0){loadsong2(playlist[0]);currIndex = 0;playpause.querySelector("i").className = "fas fa-pause";}}else{audio.pause();playpause.querySelector("i").className = "fas fa-play";}});
repeatBtn.addEventListener("click",()=>{id("repeat").style.display = "flex";});
mute.addEventListener("click",()=>{if(audio.muted){audio.muted = false;mute.querySelector("i").className = "fas fa-volume-up";}else{audio.muted = true;mute.querySelector("i").className = "fas fa-volume-mute";}});
plistBtn.addEventListener("click",()=>{var plist = id("playlist");if(plist.style.display == "none"){plist.style.display = "block";var i = 0;var intervalId = setInterval(function(){plist.style.opacity = i;if(plist.style.opacity >= 0.98){clearInterval(intervalId);}i += 0.1;},10);}else{plist.style.opacity = 0;setTimeout(function(){plist.style.display = "none";},190);}});
...
slider.addEventListener("touchmove", setAudTime);
slider.addEventListener("click", setAudTime);
slider.addEventListener("touchstart", setAudTime);
slideUI();
setBar();
